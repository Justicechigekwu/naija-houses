    ...
    showConfirm(
      {
        ...
        message: "Confirm this payment and publish the listing?",
        confirmText: "Confirm",
        cancelText: "Cancel",
      ...
      async () => {
        try {
          ...
          setRows((prev) => prev.filter((item) => item._id !== paymentId));

          showToast("Payment confirmed and removed from pending list", "success");
        } catch (e: unknown) {
          if (e instanceof AxiosError) {
            showToast(
              e.response?.data?.message || "Failed to confirm payment",
              "error"
            );
          } else {
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <div className="text-gray-600">
                        {p.listing?.publishStatus || "-"}
                      </div>
                    </td>

                    <td className="p-3">
                      <div className="font-medium">
                        {p.assignedBank?.bankName || "-"}
                      </div>
                      <div className="text-gray-600">
                        {p.assignedBank?.accountName || "-"}
                      </div>
                      <div className="text-gray-600">
                        {p.assignedBank?.accountNumber || "-"}
                      </div>
                    </td>
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          className="border bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded"
                          onClick={(e) => {
                            e.stopPropagation();
                            openDetails(p._id);
                          }}
