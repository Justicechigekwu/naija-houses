...
export default function MessagesPage() {
  ...
  const normalizePreviewMessage = (message?: MessagePreview) => {
    if (!message) return undefined;

    return {
      ...message,
      previewText: message.previewText || getMessagePreviewText(message),
    };
  };
  ...
  const fetchAndInsertChat = async (
    ...
    try {
      ...
      setChats((prev) => {
        const existing = prev.find((chat) => chat._id === chatId);
        const normalizedLastMessage = normalizePreviewMessage(message) || fetchedChat.lastMessage;
  ...
  const upsertChatFromMessage = ({
    ...
    const normalizedMessage = normalizePreviewMessage(message);
    ...
    setChats((prev) => {
      ...
      if (!existing) {
        queueMicrotask(() => {
          fetchAndInsertChat(chatId, normalizedMessage, incrementUnread);
        });
        return prev;
      }
