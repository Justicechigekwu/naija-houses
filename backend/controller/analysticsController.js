...
export const trackAnalyticsEvent = async (req, res) => {
  try {
    const { eventType, visitorId, listingId, category, subcategory, meta } =
      req.body;

    if (!eventType || !visitorId) {
      return res
        .status(400)
        .json({ message: "eventType and visitorId are required" });
    }
    ...
    return res.status(500).json({
      message: error.message || "Failed to track analytics event",
    });
  }
};
...
export const getAdminAnalyticsOverview = async (req, res) => {
  try {
    ...
    const [visitors7, visitors30, topCategories, topSubcategories] =
      await Promise.all([
